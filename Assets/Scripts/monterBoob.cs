...
public class monterBoob : MonoBehaviour {
  ...
  private void Start() {
    isAlive = true;
    InvokeRepeating("ThrowBomb", 2f,
                    6f); // Gọi hàm ThrowBomb mỗi 3 giây sau 1 giây
  }

  void ThrowBomb() {
    Vector2 spawnPosition =
        (Vector2)transform.position -
        new Vector2(1f, 0f); // Vị trí quả bom sẽ được tạo ra
    GameObject bomb =
    ...
    bombRigidbody.AddForce(new Vector2(-throwForce, 0f),
                           ForceMode2D.Impulse); // Hướng lực ném qua bên trái
